...
#     dec_dim = [128]
    ...
    if args.data_name == 'qm9':
        ...
        b_n_squeeze = 19   # 19
        a_n_node = 38
        a_n_type = len(atomic_num_list)  # 15
    ...
    dataset = NumpyTupleDataset.load(os.path.join(args.data_dir, data_file), os.path.join(args.data_dir, data_file2), transform=transform_fn)  
    print("DATASET DEF:" , dataset)
    print("DATASET SHAPE:", len(dataset))
    print("SMILES Example:" , dataset[1][0])
    print("SPECTRA SHAPE:" , dataset[3][1].shape)
    print("NODE ARRAY SHAPE:" , dataset[0][2].shape)
    print("ADJA MATRIX SHAPE:" , dataset[0][3].shape)
    print("LABEL ARRAY SHAPE" , dataset[0][4].shape)
...
#     print('Num Train-size: {}'.format(len(train)))
    ...
    for epoch in range(args.max_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            x = batch[2].to(device)  # (256,9,5)
            adj = batch[3].to(device)  # (256,4,9, 9)
            adj_normalized = rescale_adj(adj).to(device)

            ## Load the spectra instead
            ...
            # print(context_flatten)

            # Forward, backward and optimize
            z, sum_log_det_jacs, vae_para = model(adj, x, adj_normalized, context=context)
            ...
            adjm, xm = model.reverse(vae_para[2], context=context)
