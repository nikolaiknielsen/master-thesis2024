...
from mflow.models.enc_spec import SpecVEA  
...
class MoFlow(nn.Module):
    def __init__(self, hyper_params: Hyperparameters):
        ...
        self.spec_vae = SpecVEA(device=device)
...
#         self.aromaticenc = aromaticEncoder(self.enc_conv_dim, self.a_n_type, self.b_n_type-1, self.enc_linear_dim, self.a_n_node, self.b_n_type, self.a_n_type)
    ...
    def forward(self, adj, x, adj_normalized, context, device=-1):
        ...
        ### NEW ONE SPECTRA ENCODER 
        """mu, log_var = self.spec_vae.encode(x)
        print("MU:", mu)
        z = self.spec_vae.reparameterize((mu, log_var))
        vae_para = [mu, log_var, z]
        return x, z, vae_para""" 

        
        mu, log_var = self.spec_vae.encode(context)
        z = self.spec_vae.reparameterize((mu, log_var))
        vae_para = [mu, log_var, z]
    
        
       
        #### OLD ONE FOR TRAINING MOLECULAR GRAPHS
