...
from data import transform_qm9, transform_zinc250k, transform_hmdb
...
def generate_mols_interpolation_grid(model, z0=None, true_adj=None, seed=1,
                                     mols_per_row=5, device=None, delta=1.):
    np.random.seed(seed)
...
def visualize_interpolation(filepath, model, mol_smiles=None, mols_per_row=5,
                            delta=0.1, seed=1, atomic_num_list=[6, 7, 8, 9, 0], true_data=None,
    ...
    if mol_smiles is not None:  # Not used in this file
        ...
        with torch.no_grad():
            ...
            z0, _, _ = model(adj, x, adj_normalized)  # [h:(1,45), adj:(1,324)], [sum_log_det_jacs_x: (1,), sum_log_det_jacs_adj: (1,)]
...
#     img.save(filepath+"_.png") #not clear
    with open(filepath+'legend.txt', 'w') as f:
        for line in valid_mols_smiles_unique_label[:k]:
            f.write(str(line))
            f.write('\n')
...
def visualize_interpolation_between_2_points(filepath, model, mol_smiles=None, mols_per_row=11, n_interpolation=50,
                             seed=1, atomic_num_list=[6, 7, 8, 9, 0], true_data=None,
...
#             mol_index = [4546, 77955]
            ...
            z0, _, _ = model(adj0, x0, adj_normalized0)  # [h:(1,45), adj:(1,324)], [sum_log_det_jacs_x: (1,), sum_log_det_jacs_adj: (1,)]
            # z0 = np.hstack((z0[0][0].data, z0[0][1].data)).squeeze(0)
            z0[0] = z0[0].reshape(z0[0].shape[0], -1)
            z0[1] = z0[1].reshape(z0[1].shape[0], -1)
            # z0 = torch.cat((z0[0][0], z0[0][1]), dim=1).squeeze(dim=0) # h:(1,45), adj:(1,324) -> (1, 369) -> (369,)
...
#             s = '***['+s+']***'
    ...
    if data_name == 'qm9':
        ...
        psize = (100, 100)
    if k>0:
#         valid_mols_unique.insert(0, mol0)
#         valid_mols_unique.append(mol1)
#         valid_mols_smiles_unique_label.insert(0, str(1.00))#smile0)
#         valid_mols_smiles_unique_label.append(str(0.00)) #smile1)
...
#         print('model.ln_var[0]: {:.2f}, model.ln_var[1]: {:.2f}'.format(model.ln_var[0].item(), model.ln_var[1].item()))
    ...
    if args.reconstruct:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            #context_flatten = Variable(F.pad(context, (1, 200-1-L), "constant", 0), requires_grad=True)
        ...
        with open('molecules_from_spcetra.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Original SMILES', 'Reconstructed SMILES'])
            writer.writerows(smiles_pairs)
        print("SMILES pairs saved to molecules_from_spcetra.csv")
        exit(0)
    ...
    if args.int2point:
        ...
        for seed in range(args.inter_times):
            ...
            visualize_interpolation_between_2_points(filepath, model, mol_smiles=mol_smiles, mols_per_row=9,
                                                     n_interpolation=50,
    ...
    if args.intgrid:
        ...
        for seed in range(args.inter_times):
            ...
            print('saving {}'.format(filepath))
            visualize_interpolation(filepath, model, mol_smiles=mol_smiles, mols_per_row=7, delta=args.delta,
                                    atomic_num_list=atomic_num_list, seed=seed, true_data=train,
...
#                        device=device)
#         generated.extend(check_validity(adj, x, atomic_num_list, correct_validity=args.correct_validity))
        
#     for i in generated:
#         print(i)    
#     print(len(set(generated)))
#     exit(0)
...
# generation    
# python generate.py --model_dir results/zinc250k_datatest_5_15 -snapshot model_snapshot_epoch_100 --gpu 0 --data_name zinc250k --hyperparams-path moflow-params.json --batch-size 1000 --temperature 0.85 --delta 0.05 --n_experiments 3 --save_fig True --correct_validity true

# reconstruct
# python generate.py --model_dir results/qm9_model_small_size/qm9_test_8_16 -snapshot model_snapshot_epoch_5 --gpu 0 --data_name qm9  --hyperparams-path moflow-params.json --batch-size 256  --reconstruct   

# interpolation
# python generate.py --model_dir results/qm9_datatest_5_13 -snapshot model_snapshot_epoch_100 --gpu 0 --data_name qm9  --hyperparams-path moflow-params.json --batch-size 1000  --temperature 0.85   --int2point --inter_times 50  --correct_validity true 

# python generate.py --model_dir results/zinc250k_datatest_5_15 -snapshot model_snapshot_epoch_100 --gpu 0 --data_name zinc250k --hyperparams-path moflow-params.json --batch-size 1000  --temperature 0.85 --delta 5  --intgrid  --inter_times 50  --correct_validity true 

# optimization
# python optimize_property.py -snapshot model_snapshot_epoch_100  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/zinc250k_datatest_5_15 --gpu 0 --max_epochs 15  --weight_decay 1e-3  --data_name zinc250k  --hidden 32,16,  --temperature 1.0  --property_name plogp 2>&1 | tee  training_optimize_zinc250k_qed.log

# python optimize_property.py -snapshot model_snapshot_epoch_200  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/zinc250k_512t2cnn_256gnn_512-64lin_10flow_19fold_convlu2_38af-1-1mask   --gpu 0  --max_epochs 15  --weight_decay 1e-2  --data_name zinc250k  --hidden 32,16,,  --temperature 1.0  --property_name plogp 
# python optimize_property.py -snapshot model_snapshot_epoch_200  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/zinc250k_512t2cnn_256gnn_512-64lin_10flow_19fold_convlu2_38af-1-1mask   --gpu 0   --data_name zinc250k   --property_name plogp --topk 2000  --property_model_path qed_model.pt --debug false --topscore


# python optimize_property.py -snapshot model_snapshot_epoch_100  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/zinc250k_datatest_5_15   --gpu 0   --data_name zinc250k   --property_name plogp --topk 2000  --property_model_path plogp_model.pt --debug false --topscore 

# constrained optimizaiton
# python optimize_property.py -snapshot model_snapshot_epoch_100  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/qm9_datatest_5_13   --gpu 0  --max_epochs 3  --weight_decay 1e-2  --data_name qm9  --hidden 16,  --temperature 1.0  --property_name qed 

# python optimize_property.py -snapshot model_snapshot_epoch_100  --hyperparams_path moflow-params.json --batch_size 256 --model_dir results/zinc250k_datatest_5_15 --gpu 0 --data_name zinc250k --property_name plogp --topk 800 --property_model_path qed_model.pt --consopt --sim_cutoff 0.4 


# python generate.py --model_dir results/qm9_model_small_size/qm9_test_5_8 -snapshot model_snapshot_epoch_5 --gpu 0 --data_name qm9  --hyperparams-path moflow-params.json --batch-size 1000  --temperature 0.85 --delta 0.05 --n_experiments 5 --save_fig false --correct_validity true

# python generate.py --model_dir results/zinc250k_test001_19squeeze --snapshot model_snapshot_epoch_50  --gpu 1 --data_name zinc250k  --hyperparams-path moflow-params.json --batch-size 1000  --temperature 0.85 --delta 0.05 --n_experiments 3 --save_fig false --correct_validity false

# python generate.py --model_dir results/zinc250k_test001 --snapshot model_snapshot_epoch_1  --gpu 0 --data_name zinc  --hyperparams-path moflow-params.json --batch-size 1000  --temperature 0.85 --delta 5  --intgrid  --inter_times 1  --correct_validity true 2>&1 | tee qm9_visualization_intgrid.log
