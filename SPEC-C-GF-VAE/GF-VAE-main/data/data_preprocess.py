...
def parse():
    ...
    parser.add_argument('--data_name', type=str, default='qm9',
                        choices=['qm9', 'zinc250k', 'molu2024', 'hmdb'],
...
if data_name == 'qm9':
    ...
    ### ADDS THE SMILES TO THE TO DATASET 
    ### THIS IS USED TO ADD CORRECT SPECTRA LATER ON
    for idx, (node, adj, label) in enumerate(dataset):
        # print("Index:", idx, "SMILES:", smiles[idx])  # Debugging output
        enhanced_dataset.append((smiles[idx], node, adj, label))

    print("ENCHANCED:" , len(enhanced_dataset[0]))
    ...
    ### CREATES THE NODE; ADJACENCY AND LABEL ARRAYS FOR EACH MOLECULE
    dataset = result['dataset']
...
#enhanced_dataset = NumpyTupleDataset(enhanced_dataset)
enhanced_dataset = NumpyTupleDataset.convert_to_numpy_tuple_dataset(enhanced_dataset)

print("NEW DATA:", len(enhanced_dataset))
print("NEW DATA SMILES:", len(enhanced_dataset[0]))
print("NEW DATA NODES:", len(enhanced_dataset[1]))
print("NEW DATA ADJACENCY:", len(enhanced_dataset[2]))
print("NEW DATA ADJACENCY:", len(enhanced_dataset[2]))

print("FIRST MOLECULE:", enhanced_dataset[0])




NumpyTupleDataset.save(os.path.join(data_dir, '{}_{}_kekulized_ggnp.npz'.format(data_name, data_type)), enhanced_dataset)
